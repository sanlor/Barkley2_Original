...
if (move_dist > 0 or shuffle > 0) {
    scr_entity_animation_setSpeed("matthias", 1.2 - (shuffle > 0)*.5); //was .5
}
else {
    scr_entity_animation_setFrame("matthias", 0); // This is the 'standing still' frame
    scr_entity_animation_setSpeed("matthias", 0);
}
    
// Bob torso
if (move_dist > 0) {
    yoff = (floor(scr_entity_animation_getFrame("matthias")) mod 2)
}
else {
    yoff = 0
}
...
// Play step sounds
var frame = scr_entity_animation_getFrame("matthias");
var animspd = scr_entity_animation_getSpeed("matthias") * dt();
...
// Set draw script
scr_player_setDrawScript(scr_player_draw_walking_matthias); //standard)
