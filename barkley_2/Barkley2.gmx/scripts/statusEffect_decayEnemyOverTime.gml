...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_decayEnemyOverTime, 999999);
    ds_map_add(conf, "hitTimer", 10);
    ds_map_add(conf, "lifetimeSeconds", 50);
    return conf;
}
    ...
    var secs = ds_map_find_value(conf, "lifetimeSeconds");
    hitTimer -= dt();
    if (hitTimer <= 0) {
        var curHp = scr_stats_getCurrentStat(actor, STAT_CURRENT_HP);
        var maxHp = scr_stats_getEffectiveStat(actor, STAT_EFFECTIVE_MAX_HP);
        var newHp = curHp - (maxHp / secs);
        scr_stats_setCurrentStat(actor, STAT_CURRENT_HP, newHp)
        hitTimer += 10;
    }
