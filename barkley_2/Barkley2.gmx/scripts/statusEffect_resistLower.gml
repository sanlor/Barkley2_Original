// Periodic lowers target resistences by XX%.
statusEffect_this = statusEffect_resistLower;
...
else if (command == "changestat")
{
    ...
    {
        var mul = 1 - (conf[? "strength"] * global.statusResistLowerAll);
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_COSMIC, mul);
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_MENTAL, mul);
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_BIO, mul);
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_CYBER, mul);
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_ZAUBER, mul);
    }
    return 0;
}
