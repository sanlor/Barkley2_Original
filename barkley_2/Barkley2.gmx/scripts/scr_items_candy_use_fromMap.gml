/// scr_items_candy_use_fromMap(candyMap)
...
for (var i = 0; i < ds_list_size(effects); i++) 
    ...
    //show_debug_message("EFFECT = " + string(effectType));
    ...
    switch (effectType) 
        ...
        case "healOverTime":
            ...
            heal[? "healCount"] = effect[? "arg0"];
            heal[? "healAmount"] = effect[? "arg1"];
            heal[? "healInterval"] = effect[? "arg2"];
        ...
        case "gutsBoost":
            ...
            boost[? "boost"] = effect[? "arg0"];
            boost[? "timer"] = effect[? "arg1"];
            scr_statusEffect_applyNewToCombatActor(o_hoopz.id, boost);
        break;
...
if (ds_map_exists(candy, "marquee")) Marquee("queue custom", candy[? "marquee"], c_white);
...
if (gun != NULL) gun[? "pChargeCandy"] += 1;
