            ...
            if string_length(textbox_written) = string_length(textbox_goal) then
                ...
                if mouse_check_button(mb_right) then 
                    ...
                    if string_char_at(textbox_goal, textbox_position - 1) = "." then textbox_pause = 2.2;
                        ...
                        // Sound //
                        if !audio_is_playing_ext("sn_cc_wizard_talk01") and !audio_is_playing_ext("sn_cc_wizard_talk02") and !audio_is_playing_ext("sn_cc_wizard_talk03") then 
                            {
                            sound = irandom(2);
                            if sound = 0 then audio_play_sound_ext("sn_cc_wizard_talk01", 0, false);
                            else if sound = 1 then audio_play_sound_ext("sn_cc_wizard_talk02", 0, false);
                            else if sound = 2 then audio_play_sound_ext("sn_cc_wizard_talk03", 0, false);
                            }
                        }
                    }
                else
