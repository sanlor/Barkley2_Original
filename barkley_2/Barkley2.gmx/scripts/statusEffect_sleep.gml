...
if (command == "generate") {
    ...
    if conf[? "sleepEffectTimer"] > 0 conf[? "sleepEffectTimer"] -= dt();
        ...
        {
        instance_create(actor.x - choose(-irandom(8), irandom(8)), actor.y - actor.z - ailmentTotem_headqueue_get(actor) + choose(-irandom(8), irandom(8)), oDummy); //_effect_affix_narcotizing);
