...
if (argument[0] == "init")
{
    ...
    global.gunsmapFuseX = 0;
    global.gunsmapFuseY = 0;
    global.gunsmapFuseIndex = 0;
    global.gunsmapFuseType = GUN_TYPE_PISTOL;
    global.gunsmapFusePosition = 0;
    ...
    global.gunsmapGridPosition = ds_grid_create(global.gunsmapWidth, global.gunsmapHeight);
    ...
    Gunsmap("define", GUN_TYPE_PISTOL,         "Pistols",    40, make_colour_hsv(000, 255, 255));
    Gunsmap("define", GUN_TYPE_FLINTLOCK,      "Pistols",    20, make_colour_hsv(010, 255, 255));
    Gunsmap("define", GUN_TYPE_REVOLVER,       "Pistols",    28, make_colour_hsv(020, 255, 255));
    Gunsmap("define", GUN_TYPE_MAGNUM,         "Pistols",    08, make_colour_hsv(030, 255, 255));
    
    Gunsmap("define", GUN_TYPE_RIFLE,          "Rifles",     30, make_colour_hsv(040, 128, 128));
    Gunsmap("define", GUN_TYPE_MUSKET,         "Rifles",     20, make_colour_hsv(050, 128, 128));
    Gunsmap("define", GUN_TYPE_HUNTINGRIFLE,   "Rifles",     18, make_colour_hsv(060, 128, 128));
    Gunsmap("define", GUN_TYPE_SNIPERRIFLE,    "Rifles",     06, make_colour_hsv(070, 128, 128));
    Gunsmap("define", GUN_TYPE_TRANQRIFLE,     "Rifles",     10, make_colour_hsv(080, 128, 128));
    
    Gunsmap("define", GUN_TYPE_SHOTGUN,        "Shotguns",   25, make_colour_hsv(090, 255, 255));
    Gunsmap("define", GUN_TYPE_ELEPHANTGUN,    "Shotguns",   15, make_colour_hsv(100, 255, 255));
    Gunsmap("define", GUN_TYPE_DOUBLESHOTGUN,  "Shotguns",   15, make_colour_hsv(110, 255, 255));
    Gunsmap("define", GUN_TYPE_REVOLVERSHOTGUN,"Shotguns",   05, make_colour_hsv(120, 255, 255));
    
    Gunsmap("define", GUN_TYPE_SUBMACHINEGUN,  "Automatic",  18, make_colour_hsv(130, 128, 128));
    Gunsmap("define", GUN_TYPE_ASSAULTRIFLE,   "Automatic",  18, make_colour_hsv(140, 128, 128));
    Gunsmap("define", GUN_TYPE_HEAVYMACHINEGUN,"Automatic",  12, make_colour_hsv(150, 128, 128));
    Gunsmap("define", GUN_TYPE_MACHINEPISTOL,  "Automatic",  12, make_colour_hsv(160, 128, 128));
    
    Gunsmap("define", GUN_TYPE_GATLINGGUN,     "Mounted",    12, make_colour_hsv(170, 255, 255));
    Gunsmap("define", GUN_TYPE_MITRAILLEUSE,   "Mounted",    07, make_colour_hsv(180, 255, 255));
    Gunsmap("define", GUN_TYPE_MINIGUN,        "Mounted",    03, make_colour_hsv(190, 255, 255));
    Gunsmap("define", GUN_TYPE_BFG,            "Mounted",    01, make_colour_hsv(200, 255, 255));
    
    Gunsmap("define", GUN_TYPE_CROSSBOW,       "Projectile", 18, make_colour_hsv(210, 128, 128));
    Gunsmap("define", GUN_TYPE_FLAREGUN,       "Projectile", 10, make_colour_hsv(220, 128, 128));
    Gunsmap("define", GUN_TYPE_ROCKET,         "Projectile", 04, make_colour_hsv(230, 128, 128));
    Gunsmap("define", GUN_TYPE_FLAMETHROWER,   "Projectile", 03, make_colour_hsv(240, 128, 128));
    
    Gunsmap("load");
    Gunsmap("prevelence");
    //Gunsmap("generate"); // No need in b2, does on game load
}
...
else if (argument[0] == "generate")
{
    ...
    ds_grid_destroy(global.gunsmapGridPosition);
...
else if (argument[0] == "assign")
{
    ...
    if (global.gunsmapGrid[# asx, asy] == 0)
    {
        ...
        global.gunsmapGridPosition[# asx, asy] = 1 + ds_list_size(global.gunsmapTypeX[| argument[1]]);
...
else if (argument[0] == "dot to position") // 0 = "dot to position", 1 = x, 2 = y
{
    if (global.gunsmapGridPosition[# argument[1], argument[2]] != 0) 
    {
        global.gunsmapRoundX = argument[1];
        global.gunsmapRoundY = argument[2];
        return global.gunsmapGridPosition[# argument[1], argument[2]] - 1;
    }
    // First see if the grid dot is occupied
    for (dis = 1; dis < 96; dis += 1)
    {
        if (ds_grid_value_disk_exists(global.gunsmapGridTaken, argument[1], argument[2], dis, 1))
        {
            global.gunsmapRoundX = ds_grid_value_disk_x(global.gunsmapGridTaken, argument[1], argument[2], dis, 1);
            global.gunsmapRoundY = ds_grid_value_disk_y(global.gunsmapGridTaken, argument[1], argument[2], dis, 1);
            return global.gunsmapGridPosition[# global.gunsmapRoundX, global.gunsmapRoundY] - 1;
        }
    }
}
...
else if (argument[0] == "rarity by name")
{
    return global.gunsmapTypeRarity[| ds_list_find_index(global.gunsmapType, argument[1])]
}
...
else if (argument[0] == "draw") // 1 = x, 2 = y, 3 = scale, 4 = topType, 5 = topPos, 6 = botType, 7 = botPos
{
    var top = -1;
    var bot = -1;
    ...
    for (yyy = 0; yyy < hei; yyy += 1)
    {
        for (xxx = 0; xxx < wid; xxx += 1)
        {
            ...
            draw_sprite_ext(s1x1, 0, drx + (xxx * scl), dry + (yyy * scl), scl, scl, 0, global.gunsmapTypeColor[| global.gunsmapGrid[# xxx, yyy]], mapAlp);
    ...
    if (top != -1 && bot != -1)
    {
        ...
        //newTyp = Gunsmap("dot to index", chx, chy);
        //newPos = Gunsmap("dot to position", chx, chy);
        typ = Gunsmap("index to type", Gunsmap("dot to index", chx, chy));
        if (1)//global.gunsmapDebug)
        {
    ...
    if (top != -1)
    {
        ...
        draw_sprite_ext(sMenuUtilityGunMapMarker, 2, drx + gnx, dry + gny, mrkScl, mrkScl, 0, c_white, mapAlp);
    }
    ...
    if (0)
    {
        Gunsmap("fuse", top, topPos, bot, botPos, gunPow);
        newTyp = global.gunsmapFuseIndex;
        newPos = global.gunsmapFusePosition;
        gnx = (Gunsmap("x", newTyp, newPos) * scl);
        gny = (Gunsmap("y", newTyp, newPos) * scl);
        for (i = 0; i < 8; i += 1)
        {
...
else if (argument[0] == "draw64") // 1 = x, 2 = y, 3 = gunType, 4 = gunPos
{
    var top = -1;
    var bot = -1;
    var drx = argument[1];
    ...
    // Get X and Y of gun
    var gnx = (Gunsmap("x", gnt, gnp) * scl);
    var gny = (Gunsmap("y", gnt, gnp) * scl);
    
    xxx = gnx - 32;
    yyy = gny - 32;
    xxx = clamp(xxx, 0, 15);
    yyy = clamp(yyy, 0, 15);
    //show_debug_message(string(xxx + 64) + "-" + string(yyy + 64));
    // Draw occupied
    for (ycc = yyy; ycc < yyy + 63; ycc += 1)
    {
        for (xcc = xxx; xcc < xxx + 63; xcc += 1)
        {
            draw_sprite_ext(s1x1, 0, drx + ((xcc - xxx) * scl), dry + ((ycc - yyy) * scl), scl, scl, 0, global.gunsmapTypeColor[| global.gunsmapGrid[# xcc, ycc]], image_alpha);
        }
    }
    // Draw marker
    mrkScl = 1;
    draw_sprite_ext(sMenuUtilityGunMapMarker, 2, drx - xxx + gnx, dry - yyy + gny, mrkScl, mrkScl, 0, c_white, image_alpha);
}
else if (argument[0] == "fuse") // 1 = topInd, 2 = topPos, 3 = botInd, 4 = botPos, 5 = gunPow (0 to 1)
{
    // Get gun's X and Y first
    gtx = Gunsmap("x", argument[1], argument[2]);
    gty = Gunsmap("y", argument[1], argument[2]);
    gbx = Gunsmap("x", argument[3], argument[4]);
    gby = Gunsmap("y", argument[3], argument[4]);
    // Gunspower sub
    dfx = gbx - gtx;
    dfy = gby - gty;
    // Get type
    global.gunsmapFuseX = clamp(round(gtx + (dfx * argument[5])), 0, global.gunsmapWidth);
    global.gunsmapFuseY = clamp(round(gty + (dfy * argument[5])), 0, global.gunsmapHeight);
    global.gunsmapFuseIndex = Gunsmap("dot to index", global.gunsmapFuseX, global.gunsmapFuseY);
    global.gunsmapFuseType = Gunsmap("index to type", global.gunsmapFuseIndex);
    global.gunsmapFusePosition = Gunsmap("dot to position", global.gunsmapFuseX, global.gunsmapFuseY);
}
