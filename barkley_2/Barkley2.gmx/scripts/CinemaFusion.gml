/// CinemaFusion
var dts = dt_sec() * 360;
if (argument[0] == "play") // Must draw manually using event user 10
{
    if (instance_exists(oCinemaFusion))
    {
        show_debug_message("CinemaFusion('play') - Player object already exists.");
        return 0;
    }
    
    var obj = instance_create(view_xview[0], view_yview[0], oCinemaFusion);
    obj.gun1 = argument[1];
    obj.gun2 = argument[2];
    obj.gun3 = argument[3];
    return obj;
}
else if (argument[0] == "setup")
{
    // DNA - Affix
    dnaFrm = 0;
...
else if (argument[0] == "draw firepower") // 0 = firepower, 1 = x, 2 = y, 3 = damage, 4 = variance, 5 = flip
{
    ...
    if (delayRand > 0) delayRand -= 0.05 * dts;
    else { randDmg = random(meterRandsize); delayRand = 2; }
    if (meterImg < 9) meterImg += 0.02 * dts;
    if (meterSize < meterMinsize) meterSize += meterSpd * dts;
...
else if (argument[0] == "draw speed") // 0 = speed, 1 = x, 2 = y, 3 = firespeed
{
    ...
    if (meterfrm < 15) meterfrm += 0.02 * dts;
...
else if (argument[0] == "draw ammo") // 1 = x, 2 = y, 3 = type, 4 = 
{
    ...
    bulletType = Gunsmap("type to index", argument[3]);
    bullets = argument[4];
    ...
    // Step
    if(barsfrm<bulletsFrmMax){barsfrm+=0.02*dts;}
    if(textfrm<9){textfrm+=0.03*dts;}
...
else if (argument[0] == "draw type")
{
    ...
    var dry = argument[2];
    var flp = argument[4];
    showType = Gunsmap("type to index", argument[3]);
    if (textfrm < 20) textfrm += 0.03 * dts;
    if (flp)
    {
