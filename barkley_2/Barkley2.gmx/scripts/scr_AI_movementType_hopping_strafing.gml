...
if (command == "move") {
    ...
    if (point_distance(x, y, targetX, targetY) <= conf[? "stopDist"]) {
        ...
        if (instance_exists(I_AI_target)) {
            scr_AI_util_lookTowards(I_AI_target.x, I_AI_target.y, conf[? "lookSpeed"]);
        }
    }
        ...
        var dir = point_direction(x, y, targetX, targetY);// Turn towards walk direction, look towards target
    ...
    if (doMove and I_mAI_jumping = false and I_mAI_timerJump <= 0) {
        ...
        move_z = conf[? "jumpGravity"] * conf[? "jumpTime"]/2
        z -= 1;
        I_mAI_jumpDir = faceDir + conf[? "jumpAngle"] - random(conf[? "jumpAngle"]*2);
        I_mAI_jumpSpd = walkSpd;
