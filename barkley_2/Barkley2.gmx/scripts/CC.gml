...
else if (argument[0] == "hoopz player")
{
    Quest("playerCCName", "Hoopz Barkley");
    Quest("playerCCMonth", 1);     //January
    Quest("playerCCDay", 20);      //The 20th
    Quest("playerCCYear", 2040);   //Birthyear
    Quest("playerCCEra", 0);       //Clispaeth Era
    Quest("playerCCZodiac", 11);   //TODO: this should be Aquarius
    Quest("playerCCBloodType", 1); //B-type
    Quest("playerCCGender", "MaleFemaleDwarfOther");
    Quest("playerCCGenderName", "Tetrasex"); //TODO: not sure where this is stored in a normal CC
    Quest("playerCCAlignmentEthical", 1); //Neutral
    Quest("playerCCAlignmentMoral", 0);   //Good
    Quest("playerCCTarot0", 8);  //Lovers
    Quest("playerCCTarot1", 9);  //Chariot
    Quest("playerCCTarot2", 24); //Universe 
    Quest("playerCCTarot3", 26); //Babe in the Woods
    Quest("playerCCGumball", "Blue - Blueberries"); //TODO: Blue, Blueberries
    Quest("playerCCPlacenta", 3); //4 means you've had 4 Placenta Events, the Canon ones are: Multiple Choice, Likes and Faves, 
    // Events
    Quest("playerCCRune", 0);    //Not included in Canon CC
    Quest("playerCCScanner", 0); //Not included in Canon CC
    Quest("playerCCPalm", 0);    //Not included in Canon CC
    Quest("playerCCInkblot", 0); //Not included in Canon CC
    Quest("playerCCLottery", 0); //Not included in Canon CC
    for (i = 0; i < 44; i += 1) Quest("playerCCMultiple" + string(i), 0); //answers all other q's as zero
    Quest("playerCCMultiple3", 0);  //Balbanes
    Quest("playerCCMultiple21", 2); //In the air performing magnificent dunks
    Quest("playerCCMultiple31", 3); //Church to Pray - +1 Piety
    Quest("playerCCMultiple26", 3); //Sheriffs Badge
    Quest("playerCCLikes1", 2);  //Bean
    Quest("playerCCLikes2", 11); //Socks
    Quest("playerCCLikes3", 2);  //Raucous Hoot
    Quest("playerCCLikes4", 9);  //Pre-Algebra
    Quest("playerCCLikes5", 4);  //Hoop and Ball
    Quest("playerCCLikes6", 11); //Samurai
    Quest("playerCCLikes7", 9);  //Sir Dilqeg
    Quest("playerCCLikes8", 8);  //OGLE-TR-122B
    Quest("playerCCLikes9", 7);  //Nutria
    Quest("playerCCFaves1", 9);  //Gamers
    Quest("playerCCFaves2", 4);  //Bugbears
    Quest("playerCCFaves3", 7);  //Coulrophobia
    Quest("playerCCFaves4", 12); //Herbert's Healing Hands
    Quest("playerCCFaves5", 8);  //GameStop
    Quest("playerCCFaves6", 3);  //Ziggurat Electron School
    Quest("playerCCFaves7", 3);  //Alchemist Heart Baney -=Divine Comedy=-"
    Quest("ccCompleted", 1);
}
