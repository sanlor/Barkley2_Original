...
if (command == "generate") {
    scr_statusEffect_generateBasicInfo(conf, statusEffect_sh_environSlow, 5);
    return conf;
}
    ...
    var currentSlow = scr_statusEffect_getFromCombatActor(actor, statusEffect_sh_environSlow);
    if (currentSlow == -1) {
        ...
        timer = max(timer, currentSlow[? "timer"]);
        currentSlow[? "timer"] = timer;
    }
    return 0;
} 
    ...
    if (changestat == STAT_BASE_SPEED) {
        var spd;
        spd = scr_stats_getEffectiveStat( actor, STAT_BASE_SPEED );
        spd *= .9
