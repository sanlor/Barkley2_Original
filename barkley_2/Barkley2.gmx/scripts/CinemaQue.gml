...
if (argument_count == 1)
{
    ...
    else // Label
    {
        ...
        ds_list_add(global.dslCinQue, CinemaQue("whitespace", argument[0]));
        ds_map_add(global.dsmCinLab, CinemaQue("whitespace", argument[0]), ds_list_size(global.dslCinQue));
    }
}
else if (argument[0] == "whitespace") // Removes front and back whitespace froma rgument[1]
{
    var _st = argument[1];
    _st = string_replace_all(_st, chr(13), "");
    _st = string_replace_all(_st, chr(10), "");
...
else if (argument[0] == "que")
{
    ...
    //exit; // Save some time processing, B2 needs no debug
    ...
    for (var i = 0; i < array_length_2d(global.arrCinQue, _in); i += 1)
    {
        ...
        var skpOne = 0;
        if (string_pos("IF ", _tx) == 1) 
        { 
            Cinema("prune if", _tx); 
            if (string_pos("@", comNam) > 0) { Cinema("variable", "ITEM: " + string_copy(comNam, 2, string_length(comNam) - 2)); skpOne = 1; } 
            else Cinema("variable", comNam); 
        }
        while (string_pos("@", _tx) > 0)
        {
            ...
            _tx = string_delete(_tx, 1, _ps);
            if (skpOne == 0) Cinema("variable", qstNam);
            skpOne = 0;
        }
        if (string_pos("QUEST", _tx) == 1) qst = 1;
    }
}
