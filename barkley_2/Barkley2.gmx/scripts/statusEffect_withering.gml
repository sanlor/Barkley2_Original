// Periodic lowers target GLAMP by XX%.
statusEffect_this = statusEffect_withering;
...
else if (command == "changestat")
{
    ...
    {
        var low = 1 - (global.statusGlampLowerAll * conf[? "strength"]);
        statusHelper_statMultiply(actor, STAT_BASE_LUCK, low);
        statusHelper_statMultiply(actor, STAT_BASE_GUTS, low);
        statusHelper_statMultiply(actor, STAT_BASE_MIGHT, low);
        statusHelper_statMultiply(actor, STAT_BASE_PIETY, low);
        statusHelper_statMultiply(actor, STAT_BASE_AGILE, low);
    }
    return 0;
}
