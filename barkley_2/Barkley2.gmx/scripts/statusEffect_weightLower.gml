// Periodic lowers target WEIGHT XX%, lowers DEFENSE YY%.
statusEffect_this = statusEffect_weightLower;
...
{
    //var hig = 1 + (0.5 * conf[? "strength"]);
    //var low = 1 - (0.5 * conf[? "strength"]);
    if (changestat == STAT_BASE_WEIGHT)
    {
        statusHelper_statMultiply(actor, STAT_BASE_WEIGHT, 1 - (0.5 * conf[? "strength"]));
    }
    if (changestat == STATGROUP_RESISTANCES)
    {
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_NORMAL, 1 - (0.25 * conf[? "strength"]));
    }
    return 0;
}
