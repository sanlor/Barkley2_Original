// Periodic raises target knockback resistence X%, WEIGHT Y%.
statusEffect_this = statusEffect_knockGain;
...
else if (command == "changestat")
{
    ...
    {
        statusHelper_statMultiply(actor, STAT_BASE_WEIGHT, 1 + (0.25 * conf[? "strength"]));
    }
    if (changestat == STATGROUP_RESISTANCES)
    {
        statusHelper_statMultiply(actor, STAT_BASE_RESISTANCE_KNOCKBACK, 1 + (1.0 * conf[? "strength"]));
    }
    return 0;
}
