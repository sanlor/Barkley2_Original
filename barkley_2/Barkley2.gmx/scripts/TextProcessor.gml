/// TextProcessor(text)
/// Processes string replacements, then does Al Bhed (if applicable).
/// Called from o_dialog and o_choice set up scripts event

var text = argument0;

if (Quest("barbaneth"))
{
    text = string_replace_all(text, "F.A.T.E.", "F.8.");
    text = string_replace_all(text, "Wilmer", "Wilmorse");
    text = string_replace_all(text, "Dubre", "Dublorn");
    text = string_replace_all(text, "Variable", "Array");
    text = string_replace_all(text, "Zane", "Zayn");
    text = string_replace_all(text, "Zane Brain", "Zayn Brayn");
}
...
for (var i = 1; i <= length; i++) {
    ...
    if (capitalize and out_char != char) out_char = string_upper(out_char);
