...
if (argument[0] == "forum")
{
    if (argument[1] == "Troubleshooting")
    {
        Dnet("title", subforum_name[0]);
        Dnet("thread", "troubleshoot bootybass");
...
else if (argument[0] == "launch")
{
    if(scr_quest_get_state("dwarfnetAccount")){instance_create(0,0,o_dnet_control);}
    else{instance_create(0,0,o_dnet_registration_boot);}
}
...
else if (argument[0] == "timestamp") // 1 = Time
{
    ...
    {
        hours_2 = hours; //24 - hours;
        minutes_2 = minutes; //abs(0 - minutes);
        if(minutes_2==60){minutes_2=0; hours_2 +=1;}
...
else if (argument[0] == "image") // time1, time2, image, image_text
{
    ...
    {
        image = instance_create(view_xview[0] + 10, view_yview[0] + o_dnet_control.top_height + o_dnet_control.scrolling_position - 2, o_dnet_image);
...
else if (argument[0] == "thread") // 1 = scriptThread, 2 = sticky?
{
    ...
    if Dnet("clock between", "00:00", threadTime) then 
    {
        ...
        if(threadLocked)
        {
            if(Dnet("clock between", "00:00", DnetAddTime(threadTime,thread_lockTime)) ){thread.locked = 1;}
        }
