...
else if (argument[0] == "generate") // Border("generate", id, width, height);
{
    ...
    while (h > 0)
    {
        if (h <= 8) { Border("que", ind, sBorderTopBottom, 0, xxx, - 1); break; }
        iii = floor(random(14));
        siz = global.borderSize[iii];
        if (h - siz >= 0)
        {
            Border("que", ind, sBorderTopBottom, iii, xxx, - 1);
            xxx += siz;
            h -= siz;
        }
    }
    ...
    while (h > 0)
    {
        if (h <= 8) { Border("que", ind, sBorderTopBottom, 14 + 0, xxx, hei - 8 - 1); break; }
        iii = floor(random(14));
        siz = global.borderSize[iii];
        if (h - siz >= 0)
        {
            Border("que", ind, sBorderTopBottom, 14 + iii, xxx, hei - 8 - 1);
            xxx += siz;
            h -= siz;
        }
    }
    ...
    while (h > 0)
    {
        if (h <= 8) { Border("que", ind, sBorderLeftRight, 0, -1, xxx); break; }
        iii = floor(random(14));
        siz = global.borderSize[iii];
        if (h - siz >= 0)
        {
            Border("que", ind, sBorderLeftRight, iii, -1, xxx);
            xxx += siz;
            h -= siz;
        }
    }
    ...
    while (h > 0)
    {
        if (h <= 8) { Border("que", ind, sBorderLeftRight, 14 + 0, wid - 8 - 2, xxx); break; }
        iii = floor(random(14));
        siz = global.borderSize[iii];
        if (h - siz >= 0)
        {
            Border("que", ind, sBorderLeftRight, 14 + iii, wid - 8 - 2, xxx);
            xxx += siz;
            h -= siz;
        }
    }
    ...
    // Corners last
    Border("que", ind, sBorderCorners, (0 * 4) + choose(0, 1, 2, 3), - 1, - 1);
    Border("que", ind, sBorderCorners, (1 * 4) + choose(0, 1, 2, 3), -1, hei - 24 - 1);
    Border("que", ind, sBorderCorners, (2 * 4) + choose(0, 1, 2, 3), wid - 24 - 1, hei - 24 - 1);
    Border("que", ind, sBorderCorners, (3 * 4) + choose(0, 1, 2, 3), wid - 24 - 1, -1);
}
...
else if (argument[0] == "draw border") // Border("draw border", id, x, y);
{
    ...
    {
        DrawStrip(global.borderSprite[ind, i], global.borderSub[ind, i], xxx + global.borderX[ind, i], yyy + global.borderY[ind, i]);
    }
}
else if (argument[0] == "draw") // Border("draw", id, x, y);
{
    Border("draw back", argument[1], argument[2], argument[3]);
    Border("draw border", argument[1], argument[2], argument[3]);
}
