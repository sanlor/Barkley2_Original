...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_healOverTime, 9999);
    ds_map_add(conf, "healTimer", 0);
    ds_map_add(conf, "healCount", 3);
    ds_map_add(conf, "healAmount", 10);
    ds_map_add(conf, "healInterval", 5);
    return conf;
}
    ...
    var healTimer = ds_map_find_value(conf, "healTimer");
    healTimer -= dt();
    if (healTimer <= 0) {
        var healAmount = conf[? "healAmount"];
        scr_stats_setCurrentStat(actor, STAT_CURRENT_HP, scr_stats_getCurrentStat(actor, STAT_CURRENT_HP)+healAmount);
        scr_stats_createDamageIndicators(actor, -healAmount, DAMAGETYPE_NORMAL, c_green);
        healTimer += conf[? "healInterval"];
        conf[? "healCount"] -= 1;
        if (conf[? "healCount"] <= 0) {
            return true; // Remove me
        }
    }
    conf[? "healTimer"] = healTimer;
    return 0;
}
