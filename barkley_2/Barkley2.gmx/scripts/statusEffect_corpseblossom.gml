...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_corpseblossom, 15);
    ds_map_add(conf, "hitTimer", 2);
    ds_map_add(conf, "hitInterval", 5);
    ds_map_replace(conf, STAT_ATTACK_DMG_BIO, (10 + scr_time_get()));
    return conf;
}
    ...
    var current = scr_statusEffect_getFromCombatActor(actor, statusEffect_corpseblossom);
    if (current == -1) {
    ...
    // Sprout pollen //
    if irandom(100) <= 10 then instance_create(x - irandom(8) + irandom(8), y - irandom(8) + irandom(8), o_boss_corpseblossom_pollen);
