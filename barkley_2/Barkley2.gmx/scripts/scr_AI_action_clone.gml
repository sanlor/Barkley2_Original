    ...
    switch(I_atk_shot_state)
    {
        ...
        case "attack":
            ...
            if(I_atk_shot_timer>=2 && I_atk_shot_process==0)
            {
                ...
                with(_blt)
                {
                    scr_math_dirDistToVector(dir + random(20), 2 + random(3));
                    move_x = __vx;
                    move_y = __vy;
                }
                /*var knockback_dir,knockback;
                    knockback_dir = point_direction(x,y,_blt.x,_blt.y)-90;
                    knockback = 64;
                _blt.knockback_vector_x = lengthdir_x(1, knockback_dir);
                _blt.knockback_vector_y = lengthdir_y(1, knockback_dir);
                scr_stats_setCurrentStat(_blt, STAT_CURRENT_KNOCKBACK, knockback);*/
            }
