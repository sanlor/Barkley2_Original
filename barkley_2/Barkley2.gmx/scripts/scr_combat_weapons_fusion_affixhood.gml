...
global.geneMin = global.geneAffixThreshold + global.geneMinimumVariance;
global.geneMax = global.geneMaximumBase + global.geneMaximumVariance;
if (argument[0] == "load")
{
    //Default penchants
...
else if (argument[0] == "create prefix1" || argument[0] = "create prefix2" || argument[0] == "create suffix") //Make prefix table
{
    ...
    // Make maps
    if (argument[0] == "create prefix2") 
    {
        if (argument_count > 7) ds_map_add(global.prefix2HitName, argument[1], argument[7]);
        else ds_map_add(global.prefix2HitName, argument[1], argument[1]);
        ds_map_add(global.prefix2Map, argument[1], argument[6]);
        var preCol = c_bio;
        if (argument[3] == "cyber") preCol = c_cyber;
        if (argument[3] == "mental") preCol = c_mental;
        if (argument[3] == "cosmic") preCol = c_cosmic;
        if (argument[3] == "zauber") preCol = c_zauber;
        ds_map_add(global.prefix2Color, argument[1], preCol);
    }
    else if (argument[0] == "create suffix")
    {
        ds_map_add(global.suffixMap, argument[1], argument[6]);
    }
...
else if (argument[0] == "threshold") //scr_affix("threshold", affix_type, affix_name, obj_id) - TRUE / FALSE
{
    ...
    //show_debug_message("affixhood: " + argument[2] + " = " + string(_aff));
    _pts = 0; //tallies 
    ...
    if (_pts / _pol >= global.geneAffixThreshold) return 1; else return 0;
}
else if (argument[0] == "generate") //scr_affix("generate", affix_type, affix_name, object_id); - Must be executed three times
{
    ...
    for (_i = 0; _i < _gC; _i += 1)
    {
        if (gunList2D_get(gun, "aGenePenchant", _ind, _i) == global.genePrimaryValue)
        {
            ...
            ds_list_delete(global.geneListTemp, 0);  //_cou += 1;
        }
    }
    for (_i = 0; _i < _gC; _i += 1)
    {
    ...
    //See if this affix meets the threshold, if not, set as empty
    ...
    if (_thr == 0)
    {
        ...
        for (_i = 0; _i < _gC; _i += 1)
        {
            gunList2D_put(gun, "aGene", _ind, _i, 0); //ds_list_find_value(global.genePenchant[_ind, _aff], _i));
        }
...
else if (argument[0] == "init")
{
    ...
    //PREFIX2
    _ind = 1;
    global.prefix2Map = ds_map_create();
    global.prefix2HitName = ds_map_create();
    global.prefix2Color = ds_map_create();
    global.geneName[_ind] = ds_list_create();
    global.geneFamily[_ind] = ds_list_create();
    // TBC - 3 genes
    ...
    //SUFFIX
    _ind = 2;
    global.suffixMap = ds_map_create();
    global.geneName[_ind] = ds_list_create();
    global.geneFamily[_ind] = ds_list_create();
    // QUARK - 3 genes
...
else if (argument[0] == "description") // Arg1 = Name of affix
{
    ...
    if (afi == -1) afi = ds_list_find_index(global.affixName[1], argument[1]); 
    else return ds_list_find_value(global.affixDescription[0], afi);
    // Search Suffix on fail, or return Prefix 2 data
    if (afi == -1) afi = ds_list_find_index(global.affixName[2], argument[1]);
