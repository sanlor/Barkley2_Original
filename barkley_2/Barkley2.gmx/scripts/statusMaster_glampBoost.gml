...
else if (command == "changestat")
{
    ...
    {
        var hig = 1 + (global.statusGlampBoostOne * conf[? "strength"]);
        var low = 1 - (global.statusGlampLowerRest * conf[? "strength"]);
        statusHelper_statMultiply(actor, stat, hig);
        if (stat != STAT_BASE_LUCK) statusHelper_statMultiply(actor, STAT_BASE_LUCK, low);
        if (stat != STAT_BASE_GUTS) statusHelper_statMultiply(actor, STAT_BASE_GUTS, low);
        if (stat != STAT_BASE_MIGHT) statusHelper_statMultiply(actor, STAT_BASE_MIGHT, low);
        if (stat != STAT_BASE_PIETY) statusHelper_statMultiply(actor, STAT_BASE_PIETY, low);
        if (stat != STAT_BASE_AGILE) statusHelper_statMultiply(actor, STAT_BASE_AGILE, low);
    }
    return 0;
}
