...
vec3 Fetch(vec2 pos, vec2 off){
    ...
    return when_eq(floor(pos.x),0.0) *
           ...
           texture2D(gm_BaseTexture,pos.xy).rgb;
}

vec3 samples[5];
void fetchSamples(vec2 pos) {
    samples[0] = Fetch(pos, vec2(-2.0, 0.0));
    samples[1] = Fetch(pos, vec2(-1.0, 0.0));
    samples[2] = Fetch(pos, vec2( 0.0, 0.0));
    samples[3] = Fetch(pos, vec2( 1.0, 0.0));
    samples[4] = Fetch(pos, vec2( 2.0, 0.0));
}
...
vec3 Horz5(vec2 pos){
  ...
  float wa=Gaus(dst-2.0,hbloomHard);
  float wb=Gaus(dst-1.0,hbloomHard);
  float wc=Gaus(dst+0.0,hbloomHard);
  float wd=Gaus(dst+1.0,hbloomHard);
  float we=Gaus(dst+2.0,hbloomHard);
