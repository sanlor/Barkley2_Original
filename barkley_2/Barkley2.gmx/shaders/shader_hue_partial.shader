...
varying vec2 v_vTexcoord;
varying vec4 v_vColour;
uniform float argHue; // 0 - 360
uniform float argHueTarget; // 0 - 360
uniform float argHueTolerance; // 0 - 360
...
void main()
{
    ...
    //Hue shift
    vec3 hsv = RGBToHSV(retCol.x, retCol.y, retCol.z);
    // Hue tolerance does not loop, would need if I expanded the use later
    if (abs(hsv.x - argHueTarget) < argHueTolerance)
    {
        hsv.x += argHue; 
        hsv.x = mod(hsv.x, 360.0);
    }
    retCol = HSVToRGB(vec4(hsv.x, hsv.y, hsv.z, 1.0));
