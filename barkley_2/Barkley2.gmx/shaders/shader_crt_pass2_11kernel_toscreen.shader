...
void fetchSamples(vec2 pos) {
    samples[0]  = fetch(pos, vec2(-1.0,-1.0));
    samples[1]  = fetch(pos, vec2( 0.0,-1.0));
    samples[2]  = fetch(pos, vec2( 1.0,-1.0));
    
    samples[3]  = fetch(pos, vec2(-2.0, 0.0));
    samples[4]  = fetch(pos, vec2(-1.0, 0.0));
    samples[5]  = fetch(pos, vec2( 0.0, 0.0));
    samples[6]  = fetch(pos, vec2( 1.0, 0.0));
    samples[7]  = fetch(pos, vec2( 2.0, 0.0));

    samples[8]  = fetch(pos, vec2(-1.0, 0.0));
    samples[9]  = fetch(pos, vec2( 0.0, 0.0));
    samples[10] = fetch(pos, vec2( 1.0, 0.0));
}
...
vec3 Tri(vec2 pos){
  vec3 a=Horz3VScan(pos, 1);
  vec3 b=Horz5VScan(pos, 5);
  vec3 c=Horz3VScan(pos, 9);
...
vec3 Bloom(vec2 pos){
  ...
  vec3 b=Horz5(pos, 5);
  vec3 c=Horz3(pos, 9);
  float wa=BloomScan(pos,-1.0);
  float wb=BloomScan(pos, 0.0);
  float wc=BloomScan(pos, 1.0);
  return a*wa+b*wb+c*wc;}
