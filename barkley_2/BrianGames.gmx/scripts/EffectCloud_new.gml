...
else if (argument[0] == "draw") // 1 = x, 2 = y, 3 = width, 4 = height, 5 = alpha
{
    var clx = argument[1];
    var cly = argument[2];
    ...
    {
        sub = floor(random(4));
        draw_sprite_ext(sBioRainCloud3, sub, clx + (cloudX[i] * clw), cly + (cloudY[i] * clh), cloudXScl[i] * clw, cloudYScl[i] * clh, cloudRot[i], cloudCol[i], abs(cloudAlp[i] * .66) * alpha);
