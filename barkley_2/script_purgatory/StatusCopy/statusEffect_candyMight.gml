...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_candyMight, 300);
    conf[? "boost"] = 5;
    conf[? "timer"] = 30;
    return conf;
}
    ...
    var currentCBoost = scr_statusEffect_getFromCombatActor(actor, statusEffect_candyMight);
    if (currentCBoost == -1) {
        ...
        timer = max(timer, currentCBoost[? "timer"]);
        currentCBoost[? "timer"] = timer;        
    }
    return 0;
} 
    ...
    {
    scr_stats_setEffectiveStat(actor, STAT_BASE_MIGHT, scr_stats_getEffectiveStat(actor, STAT_BASE_MIGHT) + conf[? "boost"]);
    }
    return 0;
}
