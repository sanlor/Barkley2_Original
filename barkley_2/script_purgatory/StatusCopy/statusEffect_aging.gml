...
if (string(argument[0]) == "applyBullet")
{
    ...
    {
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_aging, _bullet);
...
if (command == "generate") then 
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_aging, 80);
...
// APPLY THIS STATUS EFFECT //
    ...
    var currentPagan = scr_statusEffect_getFromCombatActor(actor, statusEffect_aging);
...
// STATUS EFFECT STEP //
    ...
    if conf[? "drainSet"] = false then
        {
        conf[? "perc_agi"] = (scr_stats_getBaseStat(actor, STAT_BASE_LUCK) * (conf[? "affixPower"])) / conf[? "hitNumber"];
        conf[? "drainSet"] = true;
        }   
