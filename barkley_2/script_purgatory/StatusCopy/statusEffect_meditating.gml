...
if (string(argument[0]) == "applyBullet")
{
    ...
    if (gun[? "pChargeRatio"] == 1)
    {
        ...
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_meditating, _bullet);
        
        // SET BULLET DAMAGE TO BIO + ADD AFFIX TO THE DAMAGE AMOUNT //
        scr_stats_setAttackStat(_bullet, STAT_ATTACK_DMG_MENTAL, (scr_stats_getAttackStat(_bullet, STAT_ATTACK_DMG_NORMAL)) + gun[? "pAffix"]) 
...
if (command == "generate") then
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_meditating, 80);
    ...
    var currentComposting = scr_statusEffect_getFromCombatActor(actor, statusEffect_meditating);
    ...
    {
    scr_stats_setEffectiveStat(actor, STAT_BASE_VULN_MENTAL, scr_stats_getEffectiveStat(actor, STAT_BASE_VULN_MENTAL) + conf[? "mentalVuln"]);
    return 0;
    }
