...
if (string(argument[0]) == "applyBullet")
{
    ...
    {
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_composting, _bullet);
        scr_stats_setAttackStat(_bullet, STAT_ATTACK_DMG_BIO, (scr_stats_getAttackStat(_bullet, STAT_ATTACK_DMG_NORMAL)) + gun[? "pAffix"]) 
...
if (command == "generate") then
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_composting, 80);
    ...
    {
    scr_stats_setEffectiveStat(actor, STAT_BASE_VULN_BIO, scr_stats_getEffectiveStat(actor, STAT_BASE_VULN_BIO) + conf[? "bioVuln"]);
    return 0;
    }
