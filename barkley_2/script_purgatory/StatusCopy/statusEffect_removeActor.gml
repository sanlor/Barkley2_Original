...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_removeActor, 20);

    conf[? STAT_BASE_RESISTANCE_STAGGER] = 0;
    ...
    /* In this part, we can take into consideration resistances,
       the current_removeActor state of the CombatActor (such as if he already
       has the status effect on him), etc. */
    
    var currentRemoveActor = scr_statusEffect_getFromCombatActor(actor, statusEffect_removeActor);
    if (currentRemoveActor == -1) {
        // Apply new status effect
        scr_statusEffect_applyNewToCombatActor(actor, conf);
        
        // Effect //
        if instance_exists(actor) then with actor instance_destroy();
    }
    else {
        // Refresh time on existing
        var timer = conf[? "timer"];
        timer = max(timer, currentRemoveActor[? "timer"]);
        currentRemoveActor[? "timer"] = timer;        
    }
    return 0;
} 
