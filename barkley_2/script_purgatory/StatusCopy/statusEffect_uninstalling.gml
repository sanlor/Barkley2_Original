...
if (string(argument[0]) == "applyBullet")
{
    ...
    {
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_uninstalling, _bullet);
        ...
        // AFFIX POWER // THIS DETERMINES HOW MUCH THE CYBER RESISTANCE DROPS //
...
// APPLY THIS STATUS EFFECT //
    ...
    var currentWithering = scr_statusEffect_getFromCombatActor(actor, statusEffect_stupefying);
...
// STATUS EFFECT STEP //
    ...
    if conf[? "timer"] >= conf[? "timerStart"] then
        ...
        if (hitTimer <= 0 and hitNumber > 0) then
            {
            conf[? "resistanceCyber"] = conf[? "resistanceCyber"] - (conf[? "affixPower"]);
...
// STAT CHANGES GET PROCESSED HERE //
    ...
    if(changestat== STATGROUP_RESISTANCES)
        {
        scr_stats_setEffectiveStat(actor, STAT_BASE_RESISTANCE_CYBER, scr_stats_getEffectiveStat(actor, STAT_BASE_RESISTANCE_CYBER) + conf[? "resistanceCyber"]);   
        }
