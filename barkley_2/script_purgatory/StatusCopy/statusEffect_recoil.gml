...
if (command == "generate") {
    scr_statusEffect_generateBasicInfo(conf, statusEffect_recoil, 0.5);    
    return conf;
}
    ...
    var currentPull = scr_statusEffect_getFromCombatActor(actor, statusEffect_recoil);
    if (currentPull == -1) {
    ...
    atk[? STAT_ATTACK_KNOCKBACK] = 100 * dt();//25;
    scr_stats_performAttack(atk, actor, true, irandom(360), false, true);
    if instance_exists(o_hoopz) then scr_stats_performAttack(atk, o_hoopz, true, irandom(360), false, true);
    return 0;
}
