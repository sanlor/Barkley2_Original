...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_statSet, 80);
    return conf;
}
    ...
    {
        if(ds_map_exists(conf, STAT_BASE_GUTS)){scr_stats_setEffectiveStat(actor, STAT_BASE_GUTS, ds_map_find_value(conf, STAT_BASE_GUTS));}
        if(ds_map_exists(conf, STAT_BASE_MIGHT)){scr_stats_setEffectiveStat(actor, STAT_BASE_MIGHT, ds_map_find_value(conf, STAT_BASE_MIGHT));}
        if(ds_map_exists(conf, STAT_BASE_AGILE)){scr_stats_setEffectiveStat(actor, STAT_BASE_AGILE, ds_map_find_value(conf, STAT_BASE_AGILE));}
        if(ds_map_exists(conf, STAT_BASE_LUCK)){scr_stats_setEffectiveStat(actor, STAT_BASE_LUCK, ds_map_find_value(conf, STAT_BASE_LUCK));}
        if(ds_map_exists(conf, STAT_BASE_PIETY)){scr_stats_setEffectiveStat(actor, STAT_BASE_PIETY, ds_map_find_value(conf, STAT_BASE_PIETY));}
    }
    if(changestat== STAT_BASE_WEIGHT)
    {
        if(ds_map_exists(conf, STAT_BASE_WEIGHT)){scr_stats_setEffectiveStat(actor, STAT_BASE_WEIGHT, ds_map_find_value(conf, STAT_BASE_WEIGHT));}
    }
    return 0;
}
