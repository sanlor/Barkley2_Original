...
if (string(argument[0]) == "applyBullet")
{
    ...
    {
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_wizardly, _bullet);
        
        // SET BULLET DAMAGE TO BIO + ADD AFFIX TO THE DAMAGE AMOUNT //
        scr_stats_setAttackStat(_bullet, STAT_ATTACK_DMG_ZAUBER, (scr_stats_getAttackStat(_bullet, STAT_ATTACK_DMG_NORMAL)) + gun[? "pAffix"]) 
...
if (command == "generate") then
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_wizardly, 80);
    ...
    var currentComposting = scr_statusEffect_getFromCombatActor(actor, statusEffect_wizardly);
    ...
    {
    scr_stats_setEffectiveStat(actor, STAT_BASE_VULN_ZAUBER, scr_stats_getEffectiveStat(actor, STAT_BASE_VULN_ZAUBER) + conf[? "zauberVuln"]);
    return 0;
    }
