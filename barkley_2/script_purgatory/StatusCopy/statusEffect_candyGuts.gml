...
if (command == "generate") {
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_candyGuts, 600);
    conf[? "boost"] = 5;
    conf[? "timer"] = 30;
    return conf;
}
    ...
    var currentCBoost = scr_statusEffect_getFromCombatActor(actor, statusEffect_candyGuts);
    if (currentCBoost == -1) {
    ...
    {
    scr_stats_setEffectiveStat(actor, STAT_BASE_GUTS, scr_stats_getEffectiveStat(actor, STAT_BASE_GUTS) + conf[? "boost"]);
    }
    return 0;
}
