...
if (string(argument[0]) == "applyBullet")
{
    ...
    {
        var conf;
        conf = scr_statusEffect_addToCombatAttack(statusEffect_ionizing, _bullet);
        ...
        // AFFIX POWER // THIS DETERMINES HOW MUCH THE COSMIC RESISTANCE DROPS //
...
if (command == "generate") then 
    ...
    scr_statusEffect_generateBasicInfo(conf, statusEffect_ionizing, 80);

    // Default values so there won't be undefined variable crashes //
    conf[? "affixPower"] = 0;

    conf[? "resistanceCosmic"] = 0;
...
// APPLY THIS STATUS EFFECT //
    ...
    var currentWithering = scr_statusEffect_getFromCombatActor(actor, statusEffect_ionizing);
...
// STATUS EFFECT STEP //
    ...
    if conf[? "timer"] >= conf[? "timerStart"] then
        ...
        if (hitTimer <= 0 and hitNumber > 0) then
            {
            conf[? "resistanceCosmic"] = conf[? "resistanceCosmic"] - (conf[? "affixPower"]);
...
// STAT CHANGES GET PROCESSED HERE //
    ...
    if(changestat== STATGROUP_RESISTANCES)
        {
        scr_stats_setEffectiveStat(actor, STAT_BASE_RESISTANCE_COSMIC, scr_stats_getEffectiveStat(actor, STAT_BASE_RESISTANCE_COSMIC) + conf[? "resistanceCosmic"]);   
        }
