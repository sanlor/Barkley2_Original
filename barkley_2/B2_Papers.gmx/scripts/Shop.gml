...
if (argument[0] == "db")
{
    ...
    Shop("option", shopName, "Buy", "Rent", "Info");
    //                       Item Name              Price
    ...
    //                       Item Name         Price
    Shop("stocks", shopName, "Wilmer's Neue",  900);
    Shop("stocks", shopName, "Chickenfry Dew", 50);
    Shop("stocks", shopName, "Sweet Sweat",    750);
    Shop("stocks", shopName, "Black Licorice", 500);
    Shop("stocks", shopName, "Candy Corn",     100);
    Shop("stocks", shopName, "Ecto Poppers",   250);
    ...
    Shop("option", shopName, "Buy");
    //                       Item Name         Price
    Shop("stocks", shopName, "VIDCON_0",       900);
    Shop("stocks", shopName, "VIDCON_4",       50);
    Shop("stocks", shopName, "VIDCON_2",       750);
    Shop("stocks", shopName, "Garlics",        500);
    Shop("stocks", shopName, "Dwarf Skull",    100);
    Shop("stocks", shopName, "Magnetic Tape",  250);
    
    // GUN'SALESMAN
    shopName = "Gun'salesman Wares";
    Shop("define", shopName, "gun", sGlebFace);
    Shop("option", shopName, "Buy");
    Shop("option", shopName, "Info");
    //                       Item Name         Price
    Shop("stocks", shopName, "VIDCON_0",       900);
    Shop("stocks", shopName, "VIDCON_4",       50);
    Shop("stocks", shopName, "VIDCON_2",       750);
...
else if (argument[0] == "buy") // 1 = Shop Name, 2 = Item | Assumes you have enough NS to buy
{
    ...
    if (shpTyp == "jerkin")
    {
        Jerkin("gain", argument[2]);
        scr_money_set(scr_money_count() - shpCos[| itmInd]);
    }
    if (shpTyp == "recipe")
    {
        Candy("recipe add", argument[2]);
        scr_money_set(scr_money_count() - shpCos[| itmInd]);
    }
    if (shpTyp == "vidcon")
    {
