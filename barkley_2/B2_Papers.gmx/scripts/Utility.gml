...
// Smelt data
var smeltGain = 30; // Link to Settings
var smeltEmpty = "Are you sure you want to smelt all empty Gun'sbag guns?";
var smeltUnfaves = "Are you sure you want to smelt all unfavorited guns?";
var smeltAll = "Are you sure you want to smelt all guns?";
...
gunCol[0] = make_color_rgb(241, 217, 95); 
gunCol[1] = make_color_rgb(253, 89, 100); 
gunCol[2] = make_color_rgb(122, 83, 10); 
gunCol[3] = make_color_rgb(181, 230, 29); 
gunCol[4] = make_color_rgb(255, 128, 64); 

// Blackness of outer frame, to make the border stand out less
borderBlackness = 0.2;
textButtonBreed = "Commence"; // Breed - Commence
textButtonEmpty = "Empty"; // Breed - Empty slot
textBreedEmpty = "Select a gun.";

// DO NOT EDIT BELOW
global.utilitySpriteAlpha = 0.8 * global.utilityAlpha; // I believe this shouldn't be necessary according to the help, but it is!
...
if (argument[0] == "step")
{
    ...
    doAction = "";
    //if (keyboard_check(vk_shift)) CinemaFusion("play");
    //if (keyboard_check(vk_control)) event_user(15);
    /*
    if (keyboard_check(vk_shift)) global.utilityHue = mouse_y;
    if (keyboard_check(vk_control)) global.utilityMono = (mouse_x / SCREEN_WIDTH);
    if (keyboard_check(ord("Q"))) global.utilityAlphaBack = (mouse_x / SCREEN_WIDTH) / 2;
    if (keyboard_check(ord("Z"))) global.utilitySmelt = mouse_x / SCREEN_WIDTH;
    */
...
else if (argument[0] == "background")
{
    ...
    {
        draw_sprite_ext(s1x1, 0, (xxx * 8) - 1, argument[2] * 8, 1, argument[4] * 8, 0, _cg, (_ab + random(0.01)) * global.utilitySpriteAlpha);
    }
    for (var yyy = argument[2]; yyy < argument[2] + argument[4]; yyy += 1)
    {
        draw_sprite_ext(s1x1, 0, argument[1] * 8, (yyy * 8), argument[3] * 8, 1, 0, _cg, (_ab + random(0.01)) * global.utilitySpriteAlpha);
    }
}
...
else if (argument[0] == "grid") // grid, x, y, w, h, col
{
    ...
    var _cg = global.utilityColor[3];
    var _ba = global.utilityAlphaBack * draw_get_alpha(); //global.utilitySpriteAlpha;
    var _ga = global.utilityAlphaGlow * draw_get_alpha(); //global.utilitySpriteAlpha; // Glow alpha
    var _ea = global.utilityAlphaBorder * draw_get_alpha(); //global.utilitySpriteAlpha; // Border alpha
    if (argument_count > 5) { if (argument[5] != 3) { _cl = global.utilityColorHilight; _ba *= 8; _ga *= 4; _ea *= 4; } }
    //draw_sprite_ext(s1x1, 0, drx, dry, (argument[3] * 8) + 1, (argument[4] * 8) + 1, 0, c_lime, 0.15);
    // BACK
    draw_sprite_ext(s1x1, 0, drx + 1, dry + 1, (argument[3] * 8) - 1, (argument[4] * 8) - 1, 0, _cl, _ba);
    // GLOW
    draw_sprite_ext(s1x1, 0, drx - 1, dry - 1, (argument[3] * 8) + 3, 3, 0, _cg, _ga);
    draw_sprite_ext(s1x1, 0, drx - 1, dry + (argument[4] * 8) - 1, (argument[3] * 8) + 3, 3, 0, _cg, _ga);
    draw_sprite_ext(s1x1, 0, drx - 1, dry + 2, 3, (argument[4] * 8) - 3, 0, _cg, _ga);
    draw_sprite_ext(s1x1, 0, drx + (argument[3] * 8) - 1, dry + 2, 3, (argument[4] * 8) - 3, 0, _cg, _ga);
    // BORDER
    draw_sprite_ext(s1x1, 0, drx, dry, (argument[3] * 8) + 1, 1, 0, _cg, _ea);
    draw_sprite_ext(s1x1, 0, drx, dry + (argument[4] * 8), (argument[3] * 8) + 1, 1, 0, _cg, _ea);
    draw_sprite_ext(s1x1, 0, drx, dry + 1, 1, (argument[4] * 8) - 1, 0, _cg, _ea);
    draw_sprite_ext(s1x1, 0, drx + (argument[3] * 8), dry + 1, 1, (argument[4] * 8) - 1, 0, _cg, _ea);
}
else if (argument[0] == "linev")
{
    ...
    var dry = 16 + (argument[2] * 8);
    var _cg = global.utilityColor[3];
    draw_sprite_ext(s1x1, 0, drx - 1, dry + 2, 3, (argument[3] * 8) - 3, 0, _cg, global.utilityAlphaGlow * global.utilitySpriteAlpha);
    draw_sprite_ext(s1x1, 0, drx, dry + 1, 1, (argument[3] * 8) - 1, 0, _cg, global.utilityAlphaBorder * global.utilitySpriteAlpha);
}
else if (argument[0] == "lineh")
{
    ...
    var dry = 16 + (argument[2] * 8);
    var _cg = global.utilityColor[3];
    draw_sprite_ext(s1x1, 0, drx - 1 + 1, dry - 1, (argument[3] * 8) + 3 - 2, 3, 0, _cg, global.utilityAlphaGlow * global.utilitySpriteAlpha);
    draw_sprite_ext(s1x1, 0, drx + 1, dry, (argument[3] * 8) + 1 - 2, 1, 0, _cg, global.utilityAlphaBorder * global.utilitySpriteAlpha);
}
...
else if (argument[0] == "text") // x, y, text, sep?, wid?, xscale?, yscale?
{
    ...
    draw_text_ext(argument[1], argument[2], argument[3], _se, _wd);
    draw_set_blend_mode(bm_add);
    _al = draw_get_alpha();
    draw_set_alpha((0.04 + random(0.02)) * global.utilityAlpha);
    draw_text_ext(argument[1] + 1, argument[2], argument[3], _se, _wd);
    draw_text_ext(argument[1] - 1, argument[2], argument[3], _se, _wd);
    draw_text_ext(argument[1], argument[2] + 1, argument[3], _se, _wd);
    draw_text_ext(argument[1], argument[2] - 1, argument[3], _se, _wd);
...
else if (argument[0] == "text scale") // x, y, text, sep?, wid?, xscale?, yscale?
{
    ...
    draw_text_ext_transformed(argument[1], argument[2], argument[3], _se, _wd, _xs, _ys, 0);
    draw_set_blend_mode(bm_add);
    _al = draw_get_alpha();
    draw_set_alpha((0.04 + random(0.02)) * global.utilityAlpha);
    draw_text_ext_transformed(argument[1] + 1, argument[2], argument[3], _se, _wd, _xs, _ys, 0);
    draw_text_ext_transformed(argument[1] - 1, argument[2], argument[3], _se, _wd, _xs, _ys, 0);
    draw_text_ext_transformed(argument[1], argument[2] + 1, argument[3], _se, _wd, _xs, _ys, 0);
    draw_text_ext_transformed(argument[1], argument[2] - 1, argument[3], _se, _wd, _xs, _ys, 0);
    draw_set_alpha(_al * global.utilityAlpha);
    draw_set_blend_mode(bm_normal);
}
else if (argument[0] == "draw initial") // Fake party screen
{
    ...
    for (i = 0; i < 4 i += 1)
    {
        draw_set_alpha(alphaBG * global.utilityAlpha);
        if (Utility("mouse", dx, dy, dx + dw, dy + dh)) draw_set_color(cBG[1]);
        else draw_set_color(cBG[2]);
        if (menuSelect == i) draw_set_color(cBG[0]);
        draw_rectangle(dx, dy, dx + dw, dy + dh, 0);
        draw_set_alpha(1 * global.utilityAlpha);
        ddx = dx;
        ddy = dy;
        draw_sprite_ext(sMenuUtilityFaces, partyFace[i], dx + 4, dy, 1, 1, 0, c_lime, 1 * global.utilitySpriteAlpha);
        ddx += 68; ddy += 2;//4;
...
else if (argument[0] == "draw status")
{
    ...
    draw_sprite_ext(sMenuUtilityFaces, 0, drx + 1 + 4, dry + 0, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    ...
    drw = 6 * 8; drh = 6 * 8;
    draw_sprite_ext(sMenuUtilityDefense, 0, drx + 4, dry + 8 - 2, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    ...
    for (i = 0; i < 5; i += 1)
    {
        draw_sprite_ext(sMenuUtilityResistanceBlip, 0, drx + 3, dry + 3, 1, 1, 0, rssCol[i + 1], global.utilitySpriteAlpha);
    ...
    if (eqpGun != NULL)
    {
        Gun("draw", Gun("material", eqpGun), Gun("type", eqpGun), drx + 16 + 1, dry, 1, 1, 0, c_white, global.utilitySpriteAlpha);
        Utility("text 2c", drx + 16 + 48 + 16 - 2, dry + 20, string(Gun("weight", eqpGun)) + "~", 1, 2, 1); // Weight
        drx += 8 + 8 + 8;
        drx += 32 + 24;
    ...
    //if (keyboard_check(vk_shift)) HUD("draw scale", 277, 181, 15, 25, 0, global.utilityColor[0]);
    //else 
    image_alpha = global.utilitySpriteAlpha;
    HUD("draw scale", 277, 181, 15, 15, 1, global.utilityColor[0]);
    image_alpha = 1;
...
else if (argument[0] == "draw brain")
{
    ...
    draw_sprite_ext(sMenuBrain, 0, drx + (drw / 2) - (brnWid / 2), dry + (drh / 2) - (brnHei / 2), 1, 1, 0, c_white, global.utilitySpriteAlpha);
    pxh = (brnHei / 1440) * floor(scr_savedata_get("player.xp.questxp"));
    pxha = abs(brnHei - pxh);
    draw_sprite_part_ext(sMenuBrain, 1, 0, brnHei - pxh, brnWid, pxh, drx + (drw / 2) - (brnWid / 2), dry + (drh / 2) - (brnHei / 2) + pxha, 1, 1, c_red, global.utilitySpriteAlpha);
    pxh = brnHei * (Vidcon("unboxed total") / Vidcon("game total"));
    pxha = abs(brnHei - pxh);
    draw_sprite_part_ext(sMenuBrain, 2, 0, brnHei - pxh, brnWid, pxh, drx + (drw / 2) - (brnWid / 2), dry + (drh / 2) - (brnHei / 2) + pxha, 1, 1, c_blue, global.utilitySpriteAlpha);
    Utility("text smallc", drx + (2 * 8) + 12 + 8, dry + (21 * 8) - 3, "WORK", 1, 1, 1);
    Utility("text smallc", drx + (12 * 8) + 2 - 7, dry + (2 * 8) + 5, "PLAY", 1, 1, 1);
    
    // Experience box
    //Utility("grid", 18, 1, 6 + 5 + 5, 4);
    ////////////////////////////////////// EXPERIENCE
    ...
    lvlDir += dt() * 20;// * 20;
    if (Utility("experience to next") <= 0) draw_sprite_ext(sMenuUtilityLevelUp, 0, drx, dry, 1, 1, 0, merge_color(c_white, c_lime, 1), (0.4 + lengthdir_x(0.4, lvlDir)) * global.utilitySpriteAlpha);
...
else if (argument[0] == "draw brain level")
{
    ...
    draw_sprite_ext(sMenuUtilityHoopzFull, 0, drx + (dw / 2), dry + (dh / 2) + 4, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    ...
    for (i = 0; i < 5; i += 1)
    {
        ...
        for (xxx = 0; xxx < (8 * 8) - 2; xxx += 1 / dvv)
        {
            ...
            draw_sprite_ext(s1x1, 0, wdx, wdy, 1, 1, 0, wcl, (0.7 + random(0.1)) * global.utilitySpriteAlpha); //0.8 wave strength
        }
        draw_set_alpha(1 * global.utilityAlpha);
...
else if (argument[0] == "draw brain vidcon") ////////////////////////////////////////////////////////////////////
{
    ...
    {
        ind = vidconScroll + i;
        if (Utility("mouse", drx, dry, drx + drw, dry + drh - 1) && alpOvr == 0) hover = ind;
        ...
        if (Vidcon("have", ind)) draw_sprite_ext(sMenuUtilityVidcon, Vidcon("is boxed", ind), drx + (32.5 * 8) + 1, dry, 1, 1, 0, c_white, global.utilitySpriteAlpha);
        dry += drh;
    }
    ...
    for (i = 0; i < 2; i += 1)
    {
        ...
        {
            hil = 0;
            if (Utility("mouse", drx, dry, drx + drw, dry + drh - 1)) hil = 1;
...
{
    // argument[1] = SUBSTATES
    // "main", "jerkin", "helmet", "vidcon"
    ...
    // POCKET
    drx = 7 * 8; dry = 19 * 8;
    draw_sprite_ext(s1x1, 0, drx, dry, 4 * 8, 3 * 8, 0, c_black, 0.66 * global.utilitySpriteAlpha);
    repeat (2)
    {
    ...
    if (sbs == "main")
    {
        for (h = 0; h < 2; h += 1)
        {
            ...
            if (h == 0) Helmet("draw", "current", drx + (drw / 2), dry + 20 - 2, 1, 1, 0, c_white, global.utilitySpriteAlpha);
            else Jerkin("draw", "current", drx + (drw / 2), dry + 20 - 2, 1, 1, 0, c_white, global.utilitySpriteAlpha);
            ...
            for (i = 0; i < 6 i += 1)
            {
                draw_sprite_ext(sMenuUtilityResistanceBlip, 0, drx, dry, 1, 1, 0, rssCol[i], global.utilitySpriteAlpha);
    ...
    else
    {
        ...
        hov = -1;
        eqpDid = 0;
        for (q = 0; eqpDid < clamp(eqpAmt[ein], 1, 4); q += 1)
        {
            if (eqpAmt[ein] == 1) break;
            eai = eqpScr + q; // equip actual index
            if (eai == eqq) continue; // Skip current
            eqpDid += 1;
            if (Utility("mouse", drx, dry, drx + drw, dry + drh - 1))
            {
                hov = eai; //eqpDid;
                if (Utility("key", "action")) { if (sbs == "helmet") eqpHelNew = eai; else eqpJerNew = eai; }
            }
            dry += 5 * 8;
        }
        if (doAction == "Equip")
        ...
        //Utility("tab", 15, gdy, 5, eqn[ein]); // This is page up and down
        // All but active
        eqpDid = 0;
        for (q = 0; eqpDid < clamp(eqpAmt[ein], 1, 4); q += 1) //q < clamp(eqpAmt[ein], 1, 4);
        {
            ...
            dry += 1 * 8;
            drw = 4 * 8; drh = 5 * 8;
            Utility("grid", 15, gdy + 1, 4 + 15, 5, 3 + hil);
            if (hil == 0 && hov == eai) { draw_set_alpha(0.33 * global.utilityAlpha); Utility("grid", 15, gdy + 1, 4 + 15, 5, 4); }
            draw_set_alpha(1 * global.utilityAlpha);
            Utility("linev", 19, gdy + 1, 5);
            Jerkin("draw", eqpNam[ein, eai], drx + (drw / 2), dry + 20 - 2, 1, 1, 0, c_white, global.utilitySpriteAlpha);
        ...
        if (eqpDid == 0) // Empty
        {
            ...
            dry += 1 * 8;
            drw = 4 * 8; drh = 5 * 8;
            Utility("grid", 15, gdy + 1, 4 + 15, 5, 3);
            drx += 4 * 8;
            drw = 16 * 8; drh = 2 * 8;
...
else if (argument[0] == "draw items")
{
    ...
    {
        ind = itmScr + i;
        if (Utility("mouse", drx, dry, drx + drw, dry + drh - 1)) hover = ind;
        ...
        Utility("text 2c", drx + 4, dry + (drh / 2) + 1, itmNam, !hil, 0, 1);
...
{
    /// Select
    candyMax = scr_schematics_candy_count(); //28 max
    candyHover = -1;
    ...
    else if (actionWait == 0)
    {
        ...
        for (yyy = 0; yyy < 4; yyy += 1)
        {
            for (xxx = 0; xxx < 7; xxx += 1)
            {
                if (h < candyMax) 
                {
                    if (Utility("mouse", drx, dry, drx + (3 * 8), dry + (3 * 8))) 
                    {
                        candyHover = h;
    ...
    if (doAction == "Make")
    {
        ...
        {
            var schem = scr_schematics_candy_get(candySelect);
    ...
    for (yyy = 0; yyy < 6; yyy += 1)
    {
        for (xxx = 0; xxx < 4; xxx += 1)
        {
            if (h < pocketCapacity)
            {
                ...
                if (cdy != NULL) scr_items_draw_ext(cdy, drx + 12, dry + 12, 1, 1, 0, c_white, global.utilitySpriteAlpha); //  && pocket[h] != ""
                else Utility("text smallc", drx + (1.5 * 8) + 1, dry + (1.5 * 8) + 1, "NONE", 2, 1, 1);
            }
            else draw_sprite_ext(s1x1, 0, drx, dry, (3 * 8), (3 * 8), 0, c_black, 0.9 * global.utilitySpriteAlpha);
            h += 1;
            drx += 3 * 8;
        }
        drx -= (3 * 8) * 4;
    ...
    Jerkin("draw", "current", drx + (4.5 * 8), dry + (3 * 8) + 4, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    ...
    for (yyy = 0; yyy < 4; yyy += 1)
    {
        for (xxx = 0; xxx < 7; xxx += 1)
        {
            if (h < candyMax) 
            {
                cdy = scr_schematics_candy_get(h);
                if (h == candyHover)
                {
                    hdx = gdx + (xxx * 3);
                    hdy = gdy + (yyy * 3);
                ...
                if (h != candySelect && h != candyHover) 
                    scr_items_draw_ext(cdy, drx + 4 + 8, dry + 4 + 8, 1, 1, 0, c_white, 0.8 * global.utilitySpriteAlpha);
            }
            else draw_sprite_ext(s1x1, 0, drx, dry, (3 * 8), (3 * 8), 0, c_black, 0.9 * global.utilitySpriteAlpha);
            drx += 3 * 8;
            h += 1;
    ...
    // Select draw
    if (sdx != 0 || sdy != 0) Utility("grid", sdx, sdy, 3, 3, 4); 
    scr_items_draw_ext(cdySel, (15 + (sdx * 8)) + 4 + 8, (16 + (sdy * 8)) + 4 + 8, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    if (candyHover != candySelect && candyHover != -1)
    {
        draw_set_alpha(0.5 * global.utilityAlpha);
        Utility("grid", hdx, hdy, 3, 3, 4);
        draw_set_alpha(1 * global.utilityAlpha);
        scr_items_draw_ext(cdyHov, (15 + (hdx * 8)) + 4 + 8, (16 + (hdy * 8)) + 4 + 8, 1, 1, 0, c_white, global.utilitySpriteAlpha);
    }
    ...
    {
        scr_items_draw_ext(cdySel, drx + 4 + 16, dry + 4 + 16, 2, 2, 0, c_white, global.utilitySpriteAlpha);
        Utility("text 1b", drx + (5 * 8) + 2, dry, cdySel[? "name"], 1, 0, 0);
        Utility("text 2c", drx + (5 * 8) + 2, dry + 16 + 3, cdySel[? "desc"], 1, 0, 0, (16 * 8) - 5);
    }
...
else if (argument[0] == "draw guns smelt" || argument[0] == "draw breed select top" || argument[0] == "draw breed select bottom")
{
    ...
    if (argument[0] == "draw guns smelt")
    {
        ...
        if (gunSel != -1)
        {
            ...
            if (doAction == "All") Utility("choose", smeltAll);
            if (exeAction == "Empty")
            {
                exeAction = "";
                for (i = 5; i < 5 + Gun("gunbag amount"); i += 1)
...
else if (argument[0] == "draw guns breed")
{
    if (subscriptCreate == 0 && newScript == argument[0])
    {
        ...
        //if (brdCho[0] == brdCho[1]) brdCho[1] = -1;
    ...
    for (ind = 0; ind < 2; ind += 1)
    {
        ...
        else
        {
            ...
            draw_set_alpha(global.utilityTextAlpha * global.utilityAlpha);
            DrawGunName(drx + (13 * 8), dry, Gun("prefix1", cho[ind]), Gun("prefix2", cho[ind]), Gun("material", cho[ind]), Gun("type", cho[ind]), Gun("suffix", cho[ind]), 1);
            draw_set_alpha(1 * global.utilityAlpha);
            draw_sprite_ext(sTempGun, 3 + ind, drx + 4, dry + 20, 2, 2, 0, c_white, 0.8 * global.utilitySpriteAlpha);
            // WEIGHT
            drx = 15 + (9 * 8); dry = 16 + ((gdy + 10) * 8) - 23;
            Utility("text 1b", drx + 8, dry + 4 + 2, string(Gun("weight", cho[ind])) + "|", 0, 0, 0);
            // GUN VALUE
            drx -= (8 * 8);
    ...
    //Utility("tab", gdx, gdy, 7, "GUN'SPOWER");
    ...
    gnp = 0.5; // 0 is topmost, 1 is bottomost
    if (cho[0] != -1 && cho[1] != -1) draw_sprite_ext(s1x1, 0, drx, dry + (((16 * 8) - 9) * gnp), drw, 8, 0, merge_color(c_yellow, c_black, 0.33), 0.8 * global.utilitySpriteAlpha);
    ...
    for (i = 0; i < 2; i += 1)
    {
        ...
        if (cho[i] != -1)
        {
            ...
            for (q = 0; q < genGrp; q += 1)
            {
                ...
                ds_priority_copy(dsp, genGrpPri[i, q]);
                for (v = 0; v < genSub[q]; v += 1)
                {
                    var ltr = ds_priority_find_max(dsp);
                    ds_priority_delete_max(dsp);
    ...
    drx += 8; dry += 8;
    //draw_set_blend_mode(bm_add);
    draw_sprite_ext(s1x1, 0, drx, dry, drw, drh, 0, merge_color(c_black, c_lime, 0.2 + (lgtGrn * 0.8)), 0.8 * global.utilitySpriteAlpha);
    drx += 3 * 8;
    draw_sprite_ext(s1x1, 0, drx, dry, drw, drh, 0, merge_color(c_black, c_red, 0.2 + (lgtRed * 0.8)), 0.8 * global.utilitySpriteAlpha);
    //draw_set_blend_mode(bm_normal);
...
else if (argument[0] == "draw guns overview")
{
    ...
    for (i = 0; i < Gun("gun amount"); i += 1)
    {
        ...
        Gun("draw", Gun("material", i), Gun("type", i), drx + 16 + 1, dry, 1, 1, 0, c_white, global.utilitySpriteAlpha);
        
        Utility("text 2c", drx + 4 + 4, dry + 13, string(i + 1), 1, 1, 1); // Slot #
        Utility("text 2c", drx + 16 + 48 + 16 - 2, dry + 20, string(Gun("weight", i)) + "~", 1, 2, 1); // Weight
...
else if (argument[0] == "draw guns info") // Arg0 = grid_x_offset, Arg1 = grid_y_offset, Arg2 = Gunsmap?
{
    ...
    else
    {
        ...
        drx += 4;
        dry += 3;
        draw_set_color(global.utilityColor[0]);//colTxtHil);
        draw_set_alpha(global.utilityTextAlpha * global.utilityAlpha);
        DrawGunName(drx, dry, "type", Gun("material", gunSel), Gun("type", gunSel));
        if (gdx) Gun("draw", Gun("material", gunSel), Gun("type", gunSel), drx, dry + 20, 2, 2, 0, c_white, global.utilitySpriteAlpha);
        //show_debug_message("Utility() - GUN X Y = " + string(drx) + "x" + string(dry + 20));
        drx += 102;
        draw_set_color(global.utilityColor[0]);//colTxtHil);
        draw_set_alpha(global.utilityTextAlpha * global.utilityAlpha);
        DrawGunName(drx, dry, "prefix", Gun("prefix1", gunSel));
        if (Gun("prefix1", gunSel) != "") Utility("text 7ocs", drx + 65, dry, gunInf[0], 1, 0, 0, 112);
        dry += 24;
        draw_set_color(global.utilityColor[0]);//colTxtHil);
        draw_set_alpha(global.utilityTextAlpha * global.utilityAlpha);
        DrawGunName(drx, dry, "prefix", Gun("prefix2", gunSel));
        if (Gun("prefix2", gunSel) != "") Utility("text 7ocs", drx + 65, dry, gunInf[1], 1, 0, 0, 112);
        dry += 24;
        draw_set_color(global.utilityColor[0]);//colTxtHil);
        draw_set_alpha(global.utilityTextAlpha * global.utilityAlpha);
        DrawGunName(drx, dry, "suffix", Gun("suffix", gunSel));
        if (Gun("suffix", gunSel) != "") Utility("text 7ocs", drx + 65, dry, gunInf[2], 1, 0, 0, 112);
        draw_set_alpha(1 * global.utilityAlpha);
        
        // Gun Map
        if (gnm)
        {
            drx = 15 + ((27 + gxo) * 8); dry = 16 + (gyo * 8);
            Utility("draw gun map", 27 + gxo, 0 + gyo, 0.5, 0.3);
        ...
        Utility("text 1b", drx + 8, dry - 23 + 4 + 2, "70|", 1, 0, 0);
        ...
        drx = 15 + (gxo * 8) + 2;
        dry = 16 + (gyo * 8) + (5 * 8) + 2;
        if (gdx) draw_sprite_ext(s1x1, 0, drx, dry, 2 * 8, 3 * 8, 0, c_black, 0.5 * global.utilitySpriteAlpha);
        ...
        draw_sprite_ext(sMenuGunIcon, 0, drx - 2, dry - 2, 1, 1, 0, c_ltgray, global.utilitySpriteAlpha);
        drx = 15 + (gxo * 8) + 2 + 16;
        dry = 16 + (gyo * 8) + (5 * 8) + 2;
        for (h = 0; h < 4; h += 1)
        {
...
else if (argument[0] == "draw select gunsbag")
{
    ...
    for (i = 0; i < 20; i += 1)
    {
        if (i >= Gun("gunbag amount")) break; //blp[i] = 0;
        col = c_white;
        ...
        draw_sprite_ext(sMenuUtilityBlip, Gun("type sub", i + 5) + 1, drx + 1, dry - 3, 1, 1, 0, col, global.utilitySpriteAlpha);
        drx += 8;
    }
}
else if (argument[0] == "input select bandolier")
{
    ...
    else
    {
        if (global.utilityFocus == "main")
        {
            if (Utility("key", "left")) { gunSel -= 1; if (gunSel < 0) gunSel = Gun("gun amount") - 1; }
            if (Utility("key", "right")) { gunSel += 1; if (gunSel > Gun("gun amount") - 1) gunSel = 0; }
        }
    }
}
else if (argument[0] == "draw select bandolier")
{
    ...
    for (i = 0; i < Gun("gun amount"); i += 1)
    {
        ...
        drx = 15 + (6 * 8) + ((i * 4) * 8);
        draw_sprite_ext(sMenuUtilityBlip, Gun("type sub", i) + 1, drx + 1 - 1, dry, 1, 1, 0, col, global.utilitySpriteAlpha);
...
else if (argument[0] == "draw rename" || argument[0] == "draw promote")
{
    ...
    else // Mouse
    {
        ...
        for (i = 0; i < 40; i += 1)
        {
            ...
            if (Utility("mouse", zrx, zry, zrx + (2 * 8) - 1, zry + (2 * 8) - 1)) renSel = i;
        }
    }
...
{
    drx = 15; dry = 8 * 22;
    draw_sprite_ext(sMenuUtilityGunsbag, 0, drx + (6 * 8) + 2, dry + (3 * 8) + 1, 1, 1, 0, c_white, global.utilitySpriteAlpha); // 30, 23
    Utility("grid", 0, 20, 10, 6);
    Utility("text 1b", drx + (5 * 8), dry + (2 * 8) - 2, "Gun'sbag", 1, 1, 1);
    Utility("text 1b", drx + 29 - 2, dry + (4.5 * 8) - 2 + 3, "20 (20)", 1, 1, 1);
}
else if (argument[0] == "draw smelt")
{
    ...
    draw_sprite_ext(sMenuUtilitySmelt, 0, drx + 8, dry + 11, 1, 1, 0, c_white, 0.8 * global.utilitySpriteAlpha);
    draw_sprite_ext(sMenuUtilitySmelt, smlSub, drx + 8, dry + 11, 1, 1, 0, c_white, 0.4 * global.utilitySpriteAlpha);
    draw_set_blend_mode(bm_add);
    for (z = 0; z < smlSub; z += 1)
    {
        smtDir[z] += dt() + smtSpd[z] * 3;
        draw_sprite_ext(sMenuUtilitySmelt, z, drx + 8, dry + 11, 1, 1, 0, merge_color(c_black, c_white, (0.5 + lengthdir_x(0.5, smtDir[z])) / 30), 1 * global.utilitySpriteAlpha);
    }
    draw_set_font(global.utilityFontSmelt);
    draw_set_blend_mode(bm_normal);
...
{
    if (argument_count > 1) cho[0] = argument[1]; else cho[0] = 0;
    if (argument_count > 2) cho[1] = argument[2]; else cho[1] = 1;
    gdy = 19;
    drx = 15; dry = 16 + (gdy * 8);
    ...
    {
        var lgt = cho[lin];
        Gun("draw", Gun("material", lgt), Gun("type", lgt), drx + 1, dry, 1, 1, 0, c_white, global.utilitySpriteAlpha);
        Utility("text 2c", drx + 48 + 16 - 2, dry + 15, string(Gun("weight", lgt)) + "~", 1, 2, 0);
        DrawGunName(drx + 56 + 1 + 8 + 3, dry + 4, Gun("prefix1", lgt), Gun("prefix2", lgt), Gun("material", lgt), Gun("type", lgt), Gun("suffix", lgt), 0);
        dry += 24;
    }
}
else if (argument[0] == "draw gun map") // X, Y, XPOS, YPOS
{
    gdx = argument[1]; gdy = argument[2];
    drx = 15 + (gdx * 8); dry = 16 + (gdy * 8); drw = 8 * 8; drh = 1 * 8;
    image_alpha = global.utilitySpriteAlpha;
    Gunsmap("draw64", drx + 1, dry + 1, Gunsmap("type to index", oMenuUtilityHelper.gunTop), oMenuUtilityHelper.gunTopSel);
    image_alpha = 1;
}
else if (argument[0] == "draw gun map marker")
{
    for (_i = 0; _i < 4; _i += 1)
    {
        draw_sprite_ext(sMenuUtilityGunMapMarker, argument[3], argument[1] + lengthdir_x(1, _i * 90), argument[2] + lengthdir_y(1, _i * 90), 1, 1, 0, c_white, 1 * global.utilitySpriteAlpha);
    }
    draw_sprite_ext(sMenuUtilityGunMapMarker, argument[3], argument[1], argument[2], 1, 1, 0, c_red, 1 * global.utilitySpriteAlpha);
}
else if (argument[0] == "draw resistance") // 1 = x, 2 = y
{
    ...
    for (_i = 0; _i < _sl; _i += 1)
    {
        ...
        //draw_triangle(drx, dry, 
        //    drx + lengthdir_x(_vl, _dr), dry + lengthdir_y(_vl, _dr),
        //    drx + lengthdir_x(_vl, _dr + _dd), dry + lengthdir_y(_vl, _dr + _dd), 1);
        // Blip
        if (_i < 2) _bs = 3; else if (_i == 2 || _i == 4) _bs = 2; else _bs = 1;
        draw_sprite_ext(sMenuUtilityResistanceBlip, 0, 
            drx - 2 + lengthdir_x(_rd + _bs, _dr + (_dd / 2)), dry - 2 + lengthdir_y(_rd + _bs, _dr + (_dd / 2)), 
...
else if (argument[0] == "draw dna") // 1 = x, 2 = y, 3 = bio, 4 = cyber, 5 = cosmic, 6 = zauber
{
    ...
    while (!ds_priority_empty(todraw)) {
        ...
        draw_sprite_ext(sMenuUtilityOrb, 0,
...
else if (argument[0] == "init")
{
    ...
    global.utilitySmelt = 1;
    global.utilityTextAlpha = 0.8;
    global.utilitySpriteAlpha = 0.8;
    global.utilityAlphaBack = 0.05;
    global.utilityAlphaGlow = 0.05 * 1.5 + random(0.015);
    global.utilityAlphaBorder = 0.1 * 1.5 + random(0.03);
    global.utilityHue = 80;
    global.utilityMono = 0.15; // 0 - 1, 0 being B&W 1 being full color
    global.utilityColorHilightDirection = 0;
}
else if (argument[0] == "locals")
{
...
else if (argument[0] == "hoopz stats get") // Load in these externally later
{
    ...
    // STATUS MENU
    valNam[0] = "HP"; valStt[0] = 999; //scr_stats_getEffectiveStat(self, STAT_EFFECTIVE_MAX_HP);
    valNam[1] = "LVL"; valStt[1] = floor(scr_savedata_get("player.xp.questxp"));
    // scr_savedata_put("ustation.smelt", 999);
    // HELPER QUOTIENT = distance to clock gate (reflects how quickly you are getting exp)
...
else if (argument[0] == "hoopz stats set") // Call after level up
{
    ...
    // Utility("hoopz stats get"); // Not needed as the locals are the correct ones
}
else if (argument[0] == "hoopz weight")
{
    return eqpWei[1, eqpJer] + eqpWei[0, eqpHel];
}
...
else if (argument[0] == "reload")
{
    ...
    if (smelt >= 1)
    {
        ...
        {
            var rsp = 0;
            for (var rei = 0; rei < Gun("gun amount"); rei += 1)
            {
        ...
        else
        {
            ...
            if (Gun("ammo", gunTry) < Gun("capacity", gunTry))
            {
                ...
                //show_debug_message("set gun ammo to " + string_format(Gun("ammo", gunTry), 4, 8));
            }
        }
    }
}
else if (argument[0] == "total experience")
{
    return Vidcon("experience") + floor(scr_savedata_get("player.xp.questxp"));
}
else if (argument[0] == "experience to next")
{
    return max(0, ((scr_savedata_get("player.xp.level") - 12) * 60) - Utility("total experience"));
}
...
else if (argument[0] == "alpha")
{
    if (argument_count == 1) return global.utilityAlpha; else global.utilityAlpha = argument[1];
}
else if (argument[0] == "input")
{
    if (argument_count == 1) return utilityInput; else utilityInput = argument[1];
}
...
else if (argument[0] == "quote")
{
    return choose("A baby gun arrives.",
