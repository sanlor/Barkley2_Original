...
var threadHours = real(string_copy(threadTime, 0, 2));
var threadMinutes = real(string_copy(threadTime, 4, 2));
var postHours = real(string_copy(argument[0], 0, 2));
var postMinutes = real(string_copy(argument[0], 4, 2));
postMinutes = threadMinutes + postMinutes;
if (postMinutes >= 60) { threadHours -= 1; postMinutes -= 60; }
postHours = threadHours - postHours;
var postTime = NumberFormat(postHours) + ":" + NumberFormat(postMinutes);  
if Dnet("clock between", "00:00", argument[0]) then 
{
    // Create post object //
