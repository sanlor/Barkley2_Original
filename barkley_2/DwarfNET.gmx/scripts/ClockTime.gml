...
else
{
    ...
    {
        //Experience
        scr_savedata_put("player.experience.quest", 0);
        scr_savedata_put("player.experience.timeQuest", CLOCK_MAX div 60);
    ...
    {
        return (24 - (scr_savedata_get("clock.time") / 3600));
    }
    //-------------------
    // "display" or "time"
    //-------------------
    ...
    {  
        ...
        //Update experience
        var expTime = scr_savedata_get("player.experience.timeQuest");
        _dif = expTime - (clockTime div 60);
        _rev = abs(expTime - (60 * 24)) + 1;
        for (_p = 0; _p < _dif; _p += 1)
        {
            _expPct = (_rev / (CLOCK_MAX div 60));
            _exp = EXPERIENCE_MIN + ((EXPERIENCE_MAX - EXPERIENCE_MIN) * _expPct);
            var questExp = scr_savedata_get("player.experience.quest");
            scr_savedata_put("player.experience.quest", questExp + _exp);
            _rev += 1;
        }
        scr_savedata_put("player.experience.timeQuest", clockTime div 60);
    }
    //-------------------
    // "event"
    //-------------------
    else if (argument[0] == "event")
    {
        ...
        else //Arg1 = Quest, Arg2 = Value, Arg3 = Timer (seconds)
        {
            ...
            ds_list_add(scr_savedata_get("clock.event.timer"), argument[3]);// * 60);
            QuestTracker("note", "NOTE: ClockTime queue set for " + argument[1] + " to change to state " + string(argument[2]) + " in " + string(round(argument[3] / 60)) + " minutes, NULL");
        }
    }
    //-------------------
    // "draw"
    ...
    else if (argument[0] == "draw")
    {
        ...
        dy += 16;
        dy += 16;
        draw_text(dx, dy, "Hoopz EXP: " + string(scr_savedata_get("player.experience.quest")));
        dy += 16;
