...
// choice: add
var _cr = "|"; // Delimiter
if (argument[0] == "whitespace") // Removes front and back whitespace froma rgument[1]
{
    var _st = argument[1];
    while (string_length(_st) > 0)
    {
        var _at = 0;
        if (string_copy(_st, 1, 1) == " ") { _st = string_delete(_st, 1, 1); _at = 1; }
        if (string_copy(_st, string_length(_st), 1) == " ") { _st = string_delete(_st, string_length(_st), 1); _at = 1; }
        if (_at == 0) return _st;
    }
}
else if (argument[0] == "history")
{
...
else if (argument[0] == "que")
{
    ...
    //var _st = Cinema("whitespace", argument[1]);
    var _st = argument[1];
    if (_st == "")
    {
        //ds_list_add(global.dslCinFVC, "exit");
        //ds_list_add(global.dslCinCom, "exit");
    }
    else
    {
        //var _fv = 0;
        ...
        {
            exc[i] = Cinema("whitespace", exc[i]);
            //if (i == 0) 
            //{ 
            //    if (string_pos("IF ", exc[i]) == 0) ds_list_add(global.dslCinCom, exc[i]); 
            //    else ds_list_add(global.dslCinCom, "IF"); 
            //}
            //if (_fv == 0) { if (string_pos("IF ", exc[i]) == 0) { ds_list_add(global.dslCinFVC, exc[i]); _fv = 1; } }
...
else if (argument[0] == "init") // Call once per game
{
    ...
    global.dslCinCom = ds_list_create(); // First command (can be an IF)
    global.dslCinFVC = ds_list_create(); // First valid command (not an IF)
...
else if (argument[0] == "reset")
{
    ...
    ds_list_clear(global.dslCinCom);
    ds_list_clear(global.dslCinFVC);
...
exit;
 // Add to queue
{
    if (string_pos(_cr, argument[0]) > 0) 
    {
        ds_list_add(global.dslCinLab, 0);
        Cinema("que", argument[0]);
        //_lb = 0;
    }
    else 
    {
        ...
        ds_list_add(global.dslCinFVC, "LABEL");
        ds_list_add(global.dslCinCom, "LABEL");
    }
}
