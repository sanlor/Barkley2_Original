...
else if (argument[0] == "process") // Process line
{
    ...
    {
        chtWai = 0.0 + (string_length(txtCut) / chtSpd);
...
{
    return o_curs.x >= view_xview[0] + argument[1] && o_curs.y >= view_yview[0] + argument[2] && o_curs.x <= view_xview[0] + argument[3] && o_curs.y <= view_yview[0] + argument[4];
}
...
else if (argument[0] == "partner list")
{
    ...
    ds_list_insert(dslPar, 2 + floor(random(3)), 999); // Cdwarf
